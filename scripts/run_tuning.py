"""Run parameter tuning experiments from a protocol JSON file (TSP, JSP, or FS).

Generalized version of run_tsp_tuning.py: reads "domain" from the protocol
instead of hardcoding it, so the same runner drives TSP, JSP (scheduling),
and feature-selection tuning batches.
"""
...
def _build_config(
    ...
    domain = protocol.get("domain", "tsp")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run parameter tuning batch (TSP/JSP/FS)")
    parser.add_argument("--protocol", type=Path, required=True, help="Path to tuning protocol JSON")
    parser.add_argument(
        "--output",
        type=Path,
