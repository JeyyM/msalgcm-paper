"""Analyze TSP tuning results and select frozen parameters."""
                ...
                rows.append(
                    {
                        ...
                        "runs": len(runs),
                        "successful_runs": sum(1 for row in runs if row.get("status") == "completed"),
                        ...
                        "std_gap_percentage": statistics.pstdev(gaps) if len(gaps) > 1 else 0.0,
...
def aggregate_by_config(rows: list[dict]) -> list[dict]:
    ...
    for row in rows:
        if row.get("status") != "complete" or row.get("mean_gap_percentage") is None:
            continue
    ...
    for (algorithm, config_id), group in sorted(grouped.items()):
        mean_gaps = [float(item["mean_gap_percentage"]) for item in group]
        runtimes = [
            float(item["mean_runtime_seconds"])
            for item in group
            if item.get("mean_runtime_seconds") is not None
        ]
        aggregates.append(
            {
                "algorithm": algorithm,
                ...
                "aggregate_mean_gap_percentage": statistics.mean(mean_gaps),
                "aggregate_std_gap_percentage": statistics.pstdev(mean_gaps) if len(mean_gaps) > 1 else 0.0,
                "per_instance_mean_gap": {item["instance"]: item["mean_gap_percentage"] for item in group},
                "per_instance_best_gap": {item["instance"]: item["best_gap_percentage"] for item in group},
                "mean_runtime_seconds": statistics.mean(runtimes) if runtimes else None,
    ...
    aggregates.sort(
        key=lambda item: (
            item["aggregate_mean_gap_percentage"],
            item["aggregate_std_gap_percentage"],
...
def write_aggregate_csv(aggregates: list[dict], path: Path) -> None:
    ...
    for item in aggregates:
        row = dict(item)
        row["per_instance_mean_gap"] = json.dumps(item["per_instance_mean_gap"])
        row["per_instance_best_gap"] = json.dumps(item["per_instance_best_gap"])
...
def apply_winners_to_comparison_configs(
    ...
    for path in sorted(EXAMPLES_DIR.glob("tsp_*_comparison.json")):
...
def analyze(
    ...
    selected = {
        ...
        "tie_breakers": protocol["tie_breakers"],
        "winners": {
            algorithm: {
                "config_id": winner["config_id"],
                "config_label": winner["config_label"],
                "aggregate_mean_gap_percentage": winner["aggregate_mean_gap_percentage"],
                "per_instance_mean_gap": winner["per_instance_mean_gap"],
    ...
    if apply_configs and winners:
        ...
        # Copy to canonical location used by downstream tooling
        canonical = ROOT / "results" / "tuning" / "selected_parameters.json"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Analyze TSP tuning results")
